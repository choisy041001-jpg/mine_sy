...
```
[ 툴바: 연도/부서 필터 + 직원명 검색 + 안내 문구 | 불러오기 버튼 + 엑셀 다운로드 ]
...
<div class="page" data-page="leave">
  ...
  <div class="toolbar">
    <div class="filters">
      <select class="filter-input"><option>2026년</option>...</select>
    ...
    <div style="display:flex;gap:8px">
      <a href="#none" class="btn" onclick="document.getElementById('importEmpModal').classList.add('show')">+ 일반근로자 불러오기</a>
...
<!-- CSS #screen-intro { margin:-24px -24px; overflow-y:auto } 로 처리 — 인라인 스타일 불필요 -->
  ...
  <!-- 02 증명서 (.reverse), 03 경비, 04 FAQ (.reverse) — 동일 구조 반복 -->

  <!-- 가격 정책 -->
  <div class="lux-pricing">
    <div class="lux-pricing-eyebrow">PRICING</div>
    <h2 class="lux-pricing-title">투명하고, 단순한 가격</h2>
    <p class="lux-pricing-sub">숨겨진 비용 없습니다. 사용자 수만큼 정확하게.</p>
    <div class="lux-price-grid">
      ...
      <div class="lux-price-card">
        <div class="lux-price-label">월 사용료</div>
        <div class="lux-price-value">면 제</div>
        <div class="lux-price-tag">Zero monthly fee</div>
      </div>
      <div class="lux-price-card lux-price-card-main">
        <div class="lux-price-label">사용자당 월</div>
        <div class="lux-price-value">10,000<span>원</span></div>
        <div class="lux-price-tag">Per active user</div>
      </div>
    </div>
  </div>
...
| `.lux-pricing` | 가격 정책 섹션 | `padding:120px 80px`, 텍스트 중앙 정렬 |
| `.lux-price-grid` | 가격 카드 그리드 | `grid repeat(3,1fr)`, `gap:16px`, `max-width:800px`, `margin:0 auto` |
| `.lux-price-card` | 기본 가격 카드 | `background:#fff`, `border:1px solid #e5e5e5`, `border-radius:16px` |
| `.lux-price-card-main` | 주요 가격 카드 | `background:linear-gradient(135deg,#4F46E5,#6366F1)`, 흰 텍스트 |
    ...
    <div class="modal-body" style="padding:20px;display:flex;flex-direction:column;gap:10px">
      ...
      <a href="#none" class="btn" style="justify-content:flex-start;gap:14px;height:56px;padding:0 16px"
        ...
        <div style="text-align:left">
          ...
          <div style="font-size:11px;color:var(--text-tertiary)">현재 재직 사실을 증명하는 문서 · 국문/영문</div>
        </div>
...
| 종류 | 아이콘 배경 | 설명 |
|------|------------|------|
| 재직증명서 | `#dbeafe` (파랑) | 현재 재직 사실을 증명하는 문서 · 국문/영문 |
| 경력증명서 | `#fce7f3` (핑크) | 입사일·직위·담당업무 등 경력 증명 · 국문/영문 |
...
openEmpCertStep2(type)   // empCertStep1Bg 닫기 → empCertStep2Title = type+'신청' → empCertStep2Bg.show
backToStep1()            // empCertStep2Bg 닫기 → empCertStep1Bg.show
closeEmpCertModal()      // 두 모달 모두 닫기
submitEmpCert()          // closeEmpCertModal() + showToastV7('증명서 신청이 완료되었습니다...')
```
